...
$rs_pay_check = $con->query("select * from payments where payment_id = '".$_POST['razorpay_payment_id']."' and order_parent_id = '".$_SESSION['merchant_order_id']."'");
$rows_pay_check = $rs_pay_check->num_rows;
if ($rows_pay_check > 0) {
    $html = "<script>setTimeout(function(){ window.location = '../../order_history.php'; }, 1000);</script>";
    $html = "Redirecting to Wesite...";
}else{
...
?>
<input type="hidden" class="hiddenuname" value="<?php echo $card_hold_name; ?>">
<input type="hidden" class="hiddenueamil" value="<?php echo $email; ?>">
<input type="hidden" class="hiddenuId" value="<?php echo $_SESSION['uid']; ?>">
...
if ($success === true)
{
	$query_trans="insert into payments(payment_id,order_parent_id,order_id,signature_hash,creation_date) VALUES('".$_POST['razorpay_payment_id']."','".$_SESSION['merchant_order_id']."','".$_SESSION['razorpay_order_id']."','".$_POST['razorpay_signature']."','$creation_date')";
    ...
    success: function(response)
    {
      ...
      console.log(duces.ResultData);
        ...
        function unsetProCookies(pids,odr_id){
            pids = pids.replace(/['"]+/g, '');
            var url = "../../include/action.php?id=delete-cart-item-order";
